...
from ..config import STEMS_DIR, get_stem_types
...
class StemSeparator:
    ...
    # Base mapping from audio-separator output naming patterns to our stem types
    _BASE_STEM_MAP = {
        "vocals": "Vocals",
        "drums": "Drums",
        "bass": "Bass",
    ...
    # Extended mapping for 6-stem models — guitar and piano get their own types
    _6S_STEM_MAP = {
        **_BASE_STEM_MAP,
        "guitar": "Guitar",
        "piano": "Piano",
    ...
    def __init__(self, model_name: str = "htdemucs_ft", output_dir: Path | None = None):
        ...
        # Use extended map for 6-stem models
        self.stem_map = self._6S_STEM_MAP if "6s" in model_name else self._BASE_STEM_MAP
        self._separator = None

    def load_model(self) -> None:
        """Pre-load the separation model. Call before separate() to show download progress."""
        ...
        self._separator.load_model(model_file)

    def separate(self, audio_path: Path) -> dict[str, Path]:
        """Separate an audio file into stems.

        Output structure: {output_dir}/{song_name}/{stem_type.lower()}.wav
        Returns dict mapping stem type to path.
        """
        ...
        temp_out = self.output_dir / "_temp_separation"
        temp_out.mkdir(exist_ok=True)
        self._separator.output_dir = str(temp_out)
        ...
        # Organize into per-song folder structure
