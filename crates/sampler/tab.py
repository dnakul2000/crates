...
from ..config import PACKS_DIR, STEMS_DIR, session
...
class PadGrid(QWidget):
    """Pad grid with bank selector and audition playback. Adapts to device profile."""
    ...
    def __init__(self, parent=None):
        ...
        # Build pad grid based on device profile
        grid = QGridLayout()
        grid.setSpacing(6)
...
class GenerateWorker(BaseWorker):
    ...
    def _do_generate(self):
        ...
        for stem_type, files in self.stem_files.items():
            for file_path in files:
                ...
                self.progress.emit(
                    ...
                    f"Analyzing {stem_type}: {file_path.name} — detecting BPM, key, onsets...",
                )
                ...
                try:
                    ...
                    self.progress.emit(
                        processed, total_files,
                        f"Chopping {stem_type}: {file_path.name} "
                    ...
                    self.progress.emit(
                        processed, total_files,
                        f"Applying effects to {len(chops)} {stem_type.lower()} chops...",
                    )
        ...
        if len(all_chops) > self.max_samples:
            ...
            all_chops = select_diverse_chops(all_chops, self.max_samples, device)
...
class SamplerTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.preset_description.setProperty("role", "description")
        ...
        # Per-stem effect overrides (collapsible sections)
        effects_label = QLabel("Effect Overrides:")
        effects_label.setProperty("section", True)
        center_layout.addWidget(effects_label)

        self.stem_effect_sliders: dict[str, dict[str, QSlider]] = {}
        self._effect_value_labels: dict[str, dict[str, QLabel]] = {}
        self._effect_sections: dict[str, CollapsibleSection] = {}
        from ..config import STEM_TYPES, STEM_TYPES_6
        all_stem_types = list(dict.fromkeys(STEM_TYPES + STEM_TYPES_6))

        effects_scroll_content = QWidget()
        effects_scroll_layout = QVBoxLayout(effects_scroll_content)
        effects_scroll_layout.setContentsMargins(0, 0, 0, 0)
        effects_scroll_layout.setSpacing(0)

        for stem in all_stem_types:
            section = CollapsibleSection(stem.capitalize())
            ...
            for effect_name in ["Reverb", "Compress", "Saturate"]:
                ...
                lbl.setProperty("role", "effect-name")
                lbl.setFixedWidth(65)
                effect_row.addWidget(lbl)
                ...
                val_lbl.setProperty("role", "value-display")
                # Update label + modified indicator on change
                slider.valueChanged.connect(
                    lambda v, l=val_lbl, s=stem.lower(), sec=section: (
                        l.setText(str(v)),
                        sec.set_modified(self._stem_has_non_default(s)),
                    )
                )
                effect_row.addWidget(slider, 1)
                effect_row.addWidget(val_lbl)
                section.content_layout.addLayout(effect_row)
            ...
            self._effect_sections[stem.lower()] = section
            effects_scroll_layout.addWidget(section)

        effects_scroll_layout.addStretch()

        scroll = QScrollArea()
        scroll.setWidget(effects_scroll_content)
        ...
        # Max samples slider (adapts to device)
        from ..devices import DEFAULT_DEVICE
        self._device = DEFAULT_DEVICE
        max_slots = self._device.max_slots

        max_row = QHBoxLayout()
        max_row.addWidget(QLabel("Max Samples:"))
        self.max_samples_slider = QSlider(Qt.Orientation.Horizontal)
        self.max_samples_slider.setRange(4, max_slots)
        self.max_samples_slider.setValue(max_slots)
        self.max_samples_slider.setSingleStep(4)
        self.max_samples_slider.setPageStep(self._device.pads_per_bank)
        self.max_samples_slider.valueChanged.connect(self._on_max_samples_changed)
        max_row.addWidget(self.max_samples_slider, 1)
        self.max_samples_label = QLabel(str(max_slots))
        self.max_samples_label.setFixedWidth(30)
        self.max_samples_label.setProperty("role", "max-samples")
        ...
        self.sample_count_label = QLabel(f"Samples: 0 / {max_slots}")
        self.sample_count_label.setProperty("role", "sample-count")
        ...
        self.export_btn.setProperty("role", "export")
        ...
        self.stats_label.setProperty("role", "stats")
    ...
    def _stem_has_non_default(self, stem_key: str) -> bool:
        """Check if any effect slider for the given stem is not at the default (50)."""
        sliders = self.stem_effect_sliders.get(stem_key, {})
        return any(s.value() != 50 for s in sliders.values())

    def _on_max_samples_changed(self, value: int):
        # Snap to multiples of pads_per_bank (or half-bank for large devices)
        snap = max(4, self._device.pads_per_bank // 2)
        snapped = round(value / snap) * snap
        if snapped < snap:
            snapped = snap
        snapped = min(snapped, self._device.max_slots)
    ...
    def refresh_stems(self):
        ...
        Per-song folder structure: Stems/{SongName}/{vocals,drums,...}.wav
        """
        self.stem_tree.clear()
        session.refresh_stems()

        # Build song -> {stem_type: path} from the per-song folder layout
            ...
            for song_dir in sorted(STEMS_DIR.iterdir()):
                ...
                for f in sorted(song_dir.iterdir()):
                    if f.suffix.lower() in (".wav", ".flac"):
                        ...
                        song_stems.setdefault(song_dir.name, {})[stem_type] = f
    ...
    def _on_assignment_ready(self, assignment):
        ...
        self.sample_count_label.setText(
            f"Samples: {assignment.total_assigned} / {assignment.device.max_slots}"
        )
