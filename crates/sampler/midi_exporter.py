...
def _pad_to_midi_note(bank: str, pad: int, device: DeviceProfile | None = None) -> int:
    """Convert bank + pad number to MIDI note."""
    device = device or DEFAULT_DEVICE
    bank_offset = device.bank_names.index(bank) * device.pads_per_bank
    return device.base_midi_note + bank_offset + (pad - 1)
...
def export_midi(
    ...
    track.append(mido.MetaMessage("track_name", name="Crates Pad Map", time=0))

    # Add one note per assigned pad (quarter note each, sequential)
    import numpy as np
    ticks_per_note = 480

    for bank_name in device.bank_names:
        bank = assignment.banks.get(bank_name, [])
        for i, slot in enumerate(bank):
            if slot is None:
                continue

            note = _pad_to_midi_note(bank_name, slot.pad_number, device)
...
def export_mpc_program(
    ...
    for bank_name in device.bank_names:
        ...
        for i, slot in enumerate(bank):
            ...
            ET.SubElement(instrument, "InstrumentNumber").text = str(note - device.base_midi_note)
